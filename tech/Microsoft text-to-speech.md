...
/**
  You need to create a bucket named "audio" first.

  Just invoke function text-to-audio.
  ...
  Return value：
  'https://itceb8-audio.oss.laf.run/1687944583920.mp3'
 */
...
export default async function (ctx: FunctionContext) {
  ...
  const bucketName = appid + '-audio'

  const { text } = ctx.body
  if (!text) return '参数不合法'

  // 注意这个引入的speak文本,参数可以自己调节，比如语速之类的
  const result = await ra(
    `<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="http://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="en-US">         
     <voice name="zh-CN-XiaoxiaoNeural"><prosody rate="0%" pitch="0%">${text}</prosody ></voice > </speak >`
  );
  ...
  const url = `https://${bucketName}.oss.laf.run/` + filename
